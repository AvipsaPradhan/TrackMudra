...
const ExpenseProvider = ({ children }) => {
  ...
  const addTransaction = async (transaction) => {
    try {
      const res = await axios.post('/api/v1/transaction/addtransactions', transaction);
  ...
  return (
    <ExpenseContext.Provider value={{ ...state, addTransaction, addIncome, fetchTransactions }}>
      {children}
