...
export const trainModel = async (data) => {
  const featureData = data.map(d => [
    d.open, d.high, d.low, d.close, d.volume, d.price_change, d.price_range,
    d.daily_return, d.volatility, d.moving_average_20, d.moving_average_50
  ]);
  ...
  model.add(tf.layers.dense({ units: 64, inputShape: [11], activation: 'relu' }));
