...
router.get('/details/:symbol/:date', async (req, res) => {
  try {
    ...
    if (!stockDetails) {
      return res.status(404).json({ message: 'Stock details not found for the given symbol and date' });
    }
...
router.get('/history/:symbol', async (req, res) => {
  try {
    ...
    if (!stockHistory || stockHistory.length === 0) {
      return res.status(404).json({ message: 'Stock history not found for the given symbol' });
    }

    // Sort the data in ascending order by date for the chart
    const sortedHistory = stockHistory.sort((a, b) => new Date(a.date) - new Date(b.date));
...
router.get('/recommended', requireSignIn, async (req, res) => {
  try {
    ...
    const stockPerformance = stocks.reduce((acc, stock) => {
      if (!acc[stock.symbol]) {
        acc[stock.symbol] = { symbol: stock.symbol, name: stock.name, mostRecentDetails: stock, history: [stock] };
      } else {
        acc[stock.symbol].history.push(stock);
        if (new Date(stock.date) > new Date(acc[stock.symbol].mostRecentDetails.date)) {
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch recommended stocks', error });
  }
});
